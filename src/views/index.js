...
// --- NOWY: Jawne sprawdzenie krytycznych zmiennych środowiskowych ---
const requiredEnvVars = [
  "SUPABASE_URL",
  "SUPABASE_SERVICE_ROLE_KEY",
  "OPENROUTER_API_KEY",
  // Dodaj tutaj inne krytyczne zmienne, jeśli są naprawdę niezbędne do działania aplikacji
];
...
if (missingEnvVars.length > 0) {
  console.warn(`
  ================================================================================
  ⚠️  UWAGA: Brakuje kluczowych zmiennych środowiskowych!
  Aplikacja może działać w trybie ograniczonym lub nieprawidłowo.
  Upewnij się, że plik .env.local jest poprawnie skonfigurowany
  lub zmienne są ustawione w środowisku uruchomieniowym.
  Brakujące zmienne: ${missingEnvVars.join(", ")}
  ================================================================================
  `);
  // Jeśli Supabase jest absolutnie krytyczne i aplikacja nie może bez niego działać,
  // możesz odkomentować poniższą linię, aby zapobiec uruchomieniu serwera:
  // process.exit(1);
}
// --- KONIEC NOWEGO ---
...
app.get("/api/share/:noteId", (req, res) => {
  const lesson = [...db.lessons.values()].find((item) => item.finalNote?.id === req.params.noteId || item.id === req.params.noteId);
  if (lesson) {
    return res.json({ 
  ...
  if (note) {
    return res.json({
      finalNote: { html: note.content, title: note.title, subject: note.subject },
      transcripts: [],
      homework: note.homework || null
    });
  }
...
app.post("/api/quizzes/generate", async (req, res) => {
  try {
    const teacher = await resolveTeacherContext(req, res);
    if (!teacher) return;
    if (!requireActiveLicenseForTeacher(teacher, res, "Generowanie sprawdzianu AI")) return;

    const { lessonId, noteId, numClosed, numOpen, difficulty } = req.body || {};
    let content = "";
    let subject = "";
    let classLevel = "";
    ...
    if (lessonId) {
      const lesson = getOwnedLessonOrRespond({ params: { lessonId } }, res, teacher.teacherId, teacher.schoolId);
      if (!lesson) return;
      content = lesson.finalNote?.html || lesson.sourceFiles?.[lesson.sourceFiles.length - 1]?.extractedText || "";
      subject = lesson.subject;
      classLevel = lesson.class_name;
    } else if (noteId) {
    ...
    if (!content) return res.status(400).json({ error: "Brak treści źródłowej do wygenerowania." });

    const prompt = QUIZ_PROMPT_TEMPLATE
      .replace("[TEMAT]", "Sprawdzian")
      .replace("[PRZEDMIOT]", subject || "Brak")
      .replace("[KLASA]", classLevel || "Nie podano")
      .replace("[ILOSC_ZAMKNIETYCH]", numClosed || 0)
      .replace("[ILOSC_OTWARTYCH]", numOpen || 0)
      .replace("[TRUDNOSC]", difficulty || "średni")
      .replace("[TRESC_ZRODLOWA]", content.slice(0, 20000));
    ...
    const quizData = parseJsonFromModel(out.text);
    const quiz = {
      id: randomUUID(),
      teacherId: teacher.teacherId,
      lessonId: lessonId || null,
      noteId: noteId || null,
      title: quizData.title || "Sprawdzian",
    ...
    return res.status(500).json({ error: error.message || "Błąd generowania sprawdzianu." });
  }
});
...
app.get("/api/quizzes", async (req, res) => {
  ...
  const quizzes = [...db.quizzes.values()]
    .filter(q => String(q.teacherId) === String(teacher.teacherId))
...
app.post("/api/quizzes/grade", upload.single("file"), async (req, res) => {
  try {
    const teacher = await resolveTeacherContext(req, res);
    if (!teacher) return;
    if (!requireActiveLicenseForTeacher(teacher, res, "Ocenianie AI")) return;

    const quizId = req.body.quizId;
    const quiz = db.quizzes.get(quizId);
    if (!quiz) return res.status(404).json({ error: "Sprawdzian nie znaleziony." });
    if (!req.file) return res.status(400).json({ error: "Brak pliku." });

    const promptText = GRADING_PROMPT_TEMPLATE.replace("[KLUCZ_ODPOWIEDZI]", JSON.stringify(quiz.questions));
    const out = await callOpenRouter({
      model: OPENROUTER_VISION_MODEL,
      parts: [
        { text: promptText },
    ...
    const gradingResult = parseJsonFromModel(out.text);
    return res.json({ gradingResult });
  } catch (error) {
    return res.status(500).json({ error: error.message || "Błąd oceniania sprawdzianu." });
  }
});
...
app.post("/api/homework/generate", async (req, res) => {
  try {
    const teacher = await resolveTeacherContext(req, res);
    if (!teacher) return;
    if (!requireActiveLicenseForTeacher(teacher, res, "Generowanie zadania AI")) return;
    ...
    const prompt = `Na podstawie poniższego materiału wygeneruj propozycję zadania domowego dla uczniów.
Liczba zadań/punktów: ${numTasks || 3}.
Zadanie powinno być kreatywne, sprawdzać zrozumienie tematu i zachęcać do myślenia.
Odpowiedź zwróć jako czysty tekst.
...
${content.slice(0, 15000)}`;
    ...
    return res.status(500).json({ error: error.message || "Błąd generowania zadania." });
  }
});
...
app.post("/api/lessons/:lessonId/homework", async (req, res) => {
  try {
    ...
    const homeworkText = String(req.body?.homework || "").trim();
    if (!homeworkText) return res.status(400).json({ error: "Treść zadania jest wymagana." });

    lesson.homework = homeworkText;
    db.lessons.set(lesson.id, lesson);
    await persistLessonSafe(lesson);

    const shareUrl = `${PUBLIC_APP_URL}/share/${lesson.finalNote?.id || lesson.id}`;
    return res.json({ success: true, shareUrl });
  } catch (error) {
    return res.status(500).json({ error: error.message || "Błąd zapisywania zadania." });
  }
});
...
app.post("/api/notes/:noteId/homework", async (req, res) => {
  try {
    ...
    note.homework = homeworkText;
    db.notes.set(note.id, note);

    const shareUrl = `${PUBLIC_APP_URL}/share/${note.id}`;
    return res.json({ success: true, shareUrl });
  } catch (error) {
    return res.status(500).json({ error: error.message || "Błąd zapisywania zadania." });
  }
